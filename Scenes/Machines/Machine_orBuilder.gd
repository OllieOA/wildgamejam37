...
func operate_machine(first_bit, second_bit):
	# this takes the logical OR
	
	# Make an assumption
	var longest_bit = first_bit
	var shortest_bit = second_bit
	
	if len(first_bit) < len(second_bit): 
		longest_bit = second_bit
		shortest_bit = first_bit
	
	var output_string = ""
	
	for i in range(len(shortest_bit)):
		var i_to_get = -i
		var shortest_bit_single = shortest_bit[i_to_get]
		var longest_bit_single = longest_bit[i_to_get]
		
		var bit_to_add = "0"
		if shortest_bit_single == "1" or longest_bit_single == "1":
			bit_to_add = "1"
		
		output_string = bit_to_add + output_string
		
	# Pad the output
	while len(output_string) < len(longest_bit):
